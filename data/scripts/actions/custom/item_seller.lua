...
local conf = {
    itemSellerId = 50063, -- ID do item que ativa a venda
    exhaust = 15, -- Tempo de exaustão em segundos
    lootPouchId = 23721, -- ID da loot pouch (saco de saque)
    batchSize = 200, -- Número de itens processados por lote
    batchInterval = 300 -- Intervalo entre lotes em milissegundos (100ms = 0.1s)
}

-- Lista de itens vendáveis (fonte principal)
local LootShopConfigTable = {
...
-- Tabela de venda (gerada automaticamente)
local sellingTable = {}

-- Preenche a tabela de venda a partir da configuração
for _, eachItem in ipairs(LootShopConfigTable) do
    sellingTable[eachItem.clientId] = {
        itemName = eachItem.itemName,
        sell = eachItem.sell
    }
end

-- Função para encontrar a loot pouch na Store Inbox
local function findLootPouch(player, pouchId)
    local storeInbox = player:getStoreInbox()
    if not storeInbox then
        return nil
    end

    -- Itera sobre os itens na Store Inbox
    for _, item in ipairs(storeInbox:getItems()) do
        if item:getId() == pouchId then
            return Container(item:getUniqueId()) -- Retorna o container da lootPouch
        end
    end

    return nil -- Retorna nil se não encontrar a lootPouch
end

-- Registro da ação do item
...
function itemSeller.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if player:getExhaustion("itemSellerExhaustion") > 0 then
        player:sendCancelMessage("You must wait " .. Game.getTimeInWords(player:getExhaustion("itemSellerExhaustion")) .. " to use this item again.")
        return true
    end

    -- Encontra a lootPouch na Store Inbox
    local lootPouch = findLootPouch(player, conf.lootPouchId)
    ...
    -- Verifica se há itens na lootPouch
    local totalItems = lootPouch:getItemHoldingCount()
    if totalItems < 1 then
        player:sendCancelMessage("Your loot pouch is empty.")
        player:setExhaustion("itemSellerExhaustion", conf.exhaust)
        return true
    end

    -- Cria uma lista de itens vendáveis
    local itemsToSell = {}
    local totalEarned = 0
    local totalSelled = 0

    for _, it in ipairs(lootPouch:getItems()) do
        if sellingTable[it:getId()] then
            table.insert(itemsToSell, it)
        end
    end

    if #itemsToSell == 0 then
        player:sendCancelMessage("You have no items that can be sold.")
        player:setExhaustion("itemSellerExhaustion", conf.exhaust)
        return true
    end
    ...
    local function processBatch()
        local endIndex = math.min(currentIndex + conf.batchSize - 1, #itemsToSell)
        local batchProcessed = false

        for i = currentIndex, endIndex do
            local it = itemsToSell[i]
            if it and it:getId() and sellingTable[it:getId()] then
                if it:remove() then
                    local count = it:getCount()
                    local itemPrice = sellingTable[it:getId()].sell
                    totalEarned = totalEarned + (count * itemPrice)
        ...
        if currentIndex <= #itemsToSell then
            addEvent(processBatch, conf.batchInterval) -- Agenda o próximo lote
        else
            -- Finaliza a venda
            if totalSelled > 0 then
                Bank.credit(player:getName(), totalEarned)
                player:sendTextMessage(MESSAGE_LOOK, "You sold " .. totalSelled .. " items for a total of " .. totalEarned .. " gold coins.")
            else
                player:sendCancelMessage("You have no items that can be sold.")
            end
