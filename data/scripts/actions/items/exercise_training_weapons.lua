...
local function exerciseTrainingEvent(playerId, tilePosition, weaponId, dummyId)
	...
	if not weaponCharges or weaponCharges <= 0 then
		...
		if player:isVip() and player:getItemCount(weaponId) > 0 then
			local newWeapon = player:getItemById(weaponId, true)
			if newWeapon and newWeapon:isItem() and newWeapon:hasAttribute(ITEM_ATTRIBUTE_CHARGES) then
				player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You started using another exercise weapon.")
