...
function randomItems.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    -- Verifica a capacidade do jogador antes de entregar o item
    local itemWeight = ItemType(rewardItem.id):getWeight()
    if player:getFreeCapacity() < itemWeight then
        player:sendTextMessage(MESSAGE_LOOK, "You do not have enough capacity to carry your reward.")
        return true
    end
