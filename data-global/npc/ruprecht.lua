	...
	if npcHandler:getTopic(playerId) == 0 then
		...
		if table then
			if table.itemId ~= 6496 then
				...
				npcHandler:say("So you want to exchange all your present bags for christmas tokens?", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 1 then
		if MsgContains(message, "yes") then
			if tonumber(storeTable[playerId]) == 6526 then
				local bagCount = player:getItemCount(6496)
				if bagCount > 0 then
					player:removeItem(6496, bagCount)
					player:addItem(6526, bagCount)
					npcHandler:say("Thank you! I exchanged all your " .. bagCount .. " present bags for christmas tokens.", npc, creature)
					npcHandler:setTopic(playerId, 0)
				else
					npcHandler:say("You don't have any present bags.", npc, creature)
					npcHandler:setTopic(playerId, 0)
				end
				return false
			end
