...
local function creatureSayCallback(npc, creature, type, message)
    ...
    elseif npcHandler:getTopic(playerId) == 1 then
        if player:getSex() == PLAYERSEX_MALE and not player:hasOutfit(130, 2) then
            if MsgContains(message, "yes") then
                if player:getItemCount(5903) >= 1 then
                    ...
                    player:removeItem(5903, 1) -- Remove Ferumbras' hat
                    player:addOutfitAddon(130, 2) -- Male mage addon
                    player:addOutfitAddon(141, 2) -- Male summoner addon
        ...
        elseif player:getSex() == PLAYERSEX_FEMALE then
            npcHandler:say("This task is only available for male players.", npc, creature)
        end
        npcHandler:setTopic(playerId, 0) -- Reset the topic
