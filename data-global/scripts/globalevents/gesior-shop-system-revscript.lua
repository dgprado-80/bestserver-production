...
function getResults()
    local resultId = db.storeQuery("SELECT *, `tier` FROM z_ots_comunication;")
    ...
    repeat
        ...
        if player then
            ...
            tmp.tier = result.getNumber(resultId, "tier") -- Adicionado o campo 'tier' aqui
        end
...
function shopSystemGlobalEvent.onThink(interval)
    ...
    for _, v in ipairs(results) do
        ...
        if v.exist then
            ...
            local received_item, full_weight, items_weight, item_weight = 0, 0, 0, 0
            ...
            if add_item_type == 'container' or add_item_type == 'item' then
                ...
                if not itemType then
                    ...
                    local item_weight = itemType:getWeight()
                    if add_item_type == 'container' then
                        ...
                        if not containerItemType then
                            ...
                            if itemType:isRune() then
                                items_weight = param4 * item_weight
                            else
                    ...
                    if full_weight <= free_cap then
                        if add_item_type == 'container' then
                            ...
                            if tier and tier > 0 and tier <= 10 then
                                received_item = player:addItem(param1, param2, true, 0, CONST_SLOT_WHEREEVER, tier)
                            else
