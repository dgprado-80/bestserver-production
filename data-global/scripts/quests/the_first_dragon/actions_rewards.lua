...
function finalReward.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if player:getStorageValue(Storage.FirstDragon.Feathers) ~= 2 then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You are not eligible to claim these rewards yet.")
		return true
	end

	if item.uid == 14021 and player:getStorageValue(Storage.FirstDragon.RewardMask) < os.time() then
		player:addItem(setting.name, setting.count, true)
		player:setStorageValue(Storage.FirstDragon.RewardMask, os.time() + 60 * 60 * 24 * 5)
	...
	elseif item.uid == 14022 and player:getStorageValue(Storage.FirstDragon.RewardBackpack) < os.time() then
		...
		player:setStorageValue(Storage.FirstDragon.RewardBackpack, os.time() + 60 * 60 * 24 * 365)
	elseif item.uid == 14023 and player:getStorageValue(Storage.FirstDragon.RewardFeather) < os.time() then
		player:addItem(setting.name, setting.count, true)
