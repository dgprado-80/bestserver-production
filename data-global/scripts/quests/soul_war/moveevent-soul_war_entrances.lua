...
-------------------------------------------------------
-- MEGALOMANIA ENTRANCE 2 (NOVO)
-------------------------------------------------------
local soul_war_megalomania_entrance2 = MoveEvent()

function soul_war_megalomania_entrance2.onStepIn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return false
	end
...
-- ✅ Altere a posição abaixo para o local do novo teleporte:
soul_war_megalomania_entrance2:position({ x = 32229, y = 32238, z = 6 })
soul_war_megalomania_entrance2:register()

-------------------------------------------------------
-- CLAUSTROPHOBIC INFERNO TELEPORT
-------------------------------------------------------
