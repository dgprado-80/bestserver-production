...
function iceSeal.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if item.itemid == 30221 and item.uid == 55502 then
        -- Verifica se o jogador já possui o selo de terra ativo
        if player:getStorageValue(55502) >= 1 then
            player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You already have the ice acces.")
            return false
        end
        ...
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Run find the Ice portal.You have just 10 minutes to find this")
        ...
        addEvent(function()
            if player:getStorageValue(storageKey) == currentTime then
                ...
                player:sendTextMessage(MESSAGE_GAME_HIGHLIGHT, "The Ice seal has been lost.")
            end
